...
namespace DataManager.Library.DataAccess
{
    public class SubType_Data : ISubType_Data
    {
        ...
        {
            _sql.SaveData("dbo.spDeleteSubtype",new { Id = id }, _config.GetSection("Data")[DB_Key.SO_DB_Key()]);
        }

        public void Edit(SubTypeModel type)
        {
            _sql.SaveData("dbo.EditSubType",type,_config.GetSection("Data")[DB_Key.SO_DB_Key()]);
        }

        public List<SubTypeModel> GetAll()
        {
            var stList = _sql.LoadData<SubTypeModel, dynamic>("dbo.spGetAllSubType",new { },_config.GetSection("Data")[DB_Key.SO_DB_Key()]);
            return stList;
        }

        public SubTypeModel GetById(int id)
        {
            var stList = _sql.LoadData<SubTypeModel,dynamic>("dbo.spGetSubType_ById",new { Id= id },_config.GetSection("Data")[DB_Key.SO_DB_Key()]);
        ...
        {
            var nameList = _sql.LoadData<string, dynamic>("dbo.spGetSubTypeNames",new { },_config.GetSection("Data")[DB_Key.SO_DB_Key()]);
            return nameList;
        }
    }
}
